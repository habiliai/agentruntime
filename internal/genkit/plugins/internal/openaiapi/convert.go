	...
	}
	{
		...
		if err := json.Unmarshal(jsonBytes, &c); err == nil {
			if c.MaxOutputTokens != 0 {
				chatCompletionRequest.MaxCompletionTokens = goopenai.Int(int64(c.MaxOutputTokens))
			}
	...
	if input.Output != nil &&
		...
		case ai.OutputFormatJSON:
			chatCompletionRequest.ResponseFormat = goopenai.F[goopenai.ChatCompletionNewParamsResponseFormatUnion](
				goopenai.ResponseFormatJSONObjectParam{Type: goopenai.F(goopenai.ResponseFormatJSONObjectTypeJSONObject)},
			)
		case ai.OutputFormatText:
			chatCompletionRequest.ResponseFormat = goopenai.F[goopenai.ChatCompletionNewParamsResponseFormatUnion](
				goopenai.ResponseFormatTextParam{Type: goopenai.F(goopenai.ResponseFormatTextTypeText)},
			)
		default:
...
func convertPart(part *ai.Part) (res goopenai.ChatCompletionContentPartUnionParam, err error) {
	...
	case part.IsMedia():
		res := goopenai.ImagePart(part.Text)
		res.ImageURL.Value.Detail = goopenai.F(goopenai.ChatCompletionContentPartImageImageURLDetailAuto)
	default:
...
func convertToolCall(part *ai.Part) (goopenai.ChatCompletionMessageToolCallParam, error) {
	param := goopenai.ChatCompletionMessageToolCallParam{
		ID: goopenai.String(part.ToolRequest.Ref),
		Function: goopenai.F(goopenai.ChatCompletionMessageToolCallFunctionParam{
			Name: goopenai.String(part.ToolRequest.Name),
		}),
	}
	...
	if part.ToolRequest.Input != nil {
		...
		param.Function.Value.Arguments = goopenai.String(string(args))
	} else {
		// NOTE: OpenAI API requires the Arguments field to be set even if it's empty.
		param.Function.Value.Arguments = goopenai.String("{}")
	}
